...
<div class="th bm cl">
	<div class="tf z">
		...
		<a id="filter_dateline" href="javascript:;" class="showmenu xi2{if $_GET['dateline']} xw1{/if}" onclick="showMenu(this.id)">{if $_GET['dateline'] == '86400'}{lang last_1_days}{elseif $_GET['dateline'] == '172800'}{lang last_2_days}{elseif $_GET['dateline'] == '604800'}{lang list_one_week}{elseif $_GET['dateline'] == '2592000'}{lang list_one_month}{elseif $_GET['dateline'] == '7948800'}{lang list_three_month}{else}{lang all}{lang search_any_date}{/if}</a>&nbsp;
		<a id="filter_orderby" href="javascript:;" class="showmenu xi2{if $_GET['orderby']} xw1{/if}" onclick="showMenu(this.id)">{if $_GET['orderby'] == 'dateline'}{lang list_post_time}{elseif $_GET['orderby'] == 'replies'}{lang reply}{elseif $_GET['orderby'] == 'views'}{lang views}{else}{lang list_default_sort}{/if}</a>&nbsp;
		<!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && (!$_G['setting']['forumseparator'] || !$separatepos) && !$_GET['filter']}-->
		<a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang showupgrade}</a>
		<!--{/if}-->
		<!--{if $_GET['filter'] == 'hot'}-->
	...
	<div class="tf y">
		...
		<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=digest&digest=1$forumdisplayadd[digest]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}" class="xi2{if $_GET['filter'] == 'digest'} xw1{/if}">{lang digest_posts}</a><span class="pipe">|</span>
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				...
				<!--{if !$separatepos || !$_G['setting']['forumseparator']}-->
					<tbody id="separatorline" class="emptb"><tr><!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o"></td><!--{/if}--><td></td><td></td></tr></tbody>
				<!--{/if}-->
						...
						<!--{loop $_G['forum_threadlist'] $key $thread}-->							
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								<tbody id="separatorline">
									<tr class="ts">
										...
										<td>&nbsp;</td>
										<td><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}--></td>
									</tr>
								</tbody>
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr>
									...
									<!--{/if}-->
									<td class="avatar">
										<a href="home.php?mod=space&uid=$thread[authorid]" c="1">{avatar($thread['authorid'])}</a>
									</td>
									<td class="thread_con" colspan="4">
										<div class="thread_subject $thread[folder]">
											<a class="icn" href="forum.php?mod=viewthread&tid=$thread[icontid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra" title="{if $thread['displayorder'] == 1}{lang thread_type1} - {/if}
											...
											<img src="{$_G['style']['styleimgdir']}/image.png" alt="attach_img" title="{lang attach_img}" align="absmiddle" />
											...
											<!--{/if}-->
											<!--{if $thread['weeknew']}-->
											<a href="forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost#lastpost" class="xi1">
												<img src="{$_G['style']['styleimgdir']}/new.png" alt="New" title="{lang attach_img}" align="absmiddle" />
											</a>
											<!--{/if}-->
										...
										<div class="thread_info">
											...
											<div class="by author">
												...
												<em><span{if $thread['istoday']} class="xi1"{/if}><!--{echo dgmdate($thread[dbdateline],'Y-m-d')}--></span></em>
											</div>
											<div class="by poster">
											...
											<div class="num"><a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra" class="xi2"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></a><em>$thread[allreplies]</em></div>
										</div>
									</td>
								</tr>
							</tbody>
...
<!--{if !IS_ROBOT}-->
	...
	<div id="filter_orderby_menu" class="p_pop" style="display:none">
		<ul>
			<li><a href="forum.php?mod=forumdisplay&fid=$_G[fid]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}" {if !$_GET['orderby']}class="xw1"{/if}>{lang list_default_sort}</a></li>
			...
			<li><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=reply&orderby=replies$forumdisplayadd[reply]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}" {if $_GET['orderby'] == 'replies'}class="xw1"{/if}>{lang reply}</a></li>
...
<div class="bm bw0 pgs cl">
	...
	<span {if $_G[setting][visitedforums]}id="visitedforumstmp" onmouseover="$('visitedforums').id = 'visitedforumstmp';this.id = 'visitedforums';showMenu({'ctrlid':this.id,'pos':'12'})"{/if} class="pgb y"><a href="forum.php">{lang return_index}</a></span>
