...
<?php if($param['login']) { ?>
<p style="padding-left: 58px;;">2秒后转向登录页面</p>
<script type="text/javascript">setTimeout("window.location.href='<?php echo $_G['siteurl'];?>member.php?mod=logging&action=login&referer=" + encodeURIComponent(window.location.href) + "'",2000);</script>
<?php } ?>
</div>
...
<?php } if($param['login']) { ?>
<button type="button" class="pn pnc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>');showWindow('login', '<?php echo $_G['siteurl'];?>member.php?mod=logging&action=login');"><strong>登录</strong></button>
<?php if(!$_G['setting']['bbclosed']) { ?>
<button type="button" class="pn pnc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>');window.open('<?php echo $_G['siteurl'];?>member.php?mod=<?php echo $_G['setting']['regname'];?>');"><em><?php echo $_G['setting']['reglinkname'];?></em></button>
<?php } ?>
