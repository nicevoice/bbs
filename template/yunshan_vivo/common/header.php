...
<!--{subtemplate common/header_common}-->
	...
	<!--{/if}-->
	<script type="text/javascript" src="$_G['style']['styleimgdir']/js/sea.js?{VERHASH}"></script>
    <script src="$_G['style']['styleimgdir']/js/seajs.config.js?{VERHASH}" type="text/javascript"></script>
</head>
...
<body id="nv_{$_G[basescript]}" class="pg_{CURMODULE}{if $_G['basescript'] === 'portal' && CURMODULE === 'list' && !empty($cat)} {$cat['bodycss']}{/if}" onkeydown="if(event.keyCode==27) return false;">
	...
	<!--{if empty($topic) || $topic['useheader']}-->
        ...
        <div id="hd">
			<div class="wp">
				<div class="hdc cl">
					<!--{eval $mnid = $mnid ? $mnid : getcurrentnav();}-->
				...
				<div id="nav">
					...
					<div id="unav">
					<!--{if $_G['uid']}-->
					<a href="home.php?mod=space&uid=$_G[uid]" target="_blank" title="{lang visit_my_space}" id="qmenu" class="showmenu" onMouseOver="delayShow(this, function () {showMenu({'ctrlid':'qmenu','pos':'33!','ctrlclass':'a','duration':2});showForummenu($_G[fid]);})" onClick="delayShow(this, function () {showMenu({'ctrlid':'qmenu','pos':'33!','ctrlclass':'a','duration':2});showForummenu($_G[fid]);});return false;">{$_G[member][username]}</a>
					<span class="pipe">|</span><a href="member.php?mod=logging&action=logout&formhash={FORMHASH}">{lang logout}</a>
					<!--{else}-->
					<a href="member.php?mod=logging&action=login" class="xi2 xw1">{lang login}</a>
					<span class="pipe">|</span><a href="member.php?mod={$_G[setting][regname]}" class="xi2 xw1">$_G['setting']['reglinkname']</a>
					<!--{/if}-->
					</div>
					</div>
				</div>
