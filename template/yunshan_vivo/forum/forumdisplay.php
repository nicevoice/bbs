	...
	<div id="ct" class="wp cl  ct2 ">
		...
		<div class="mn">
			<div class="bm bml pbn">
				...
				<div class="bm_h cl">
								<div class="fl_icn_g"{if !empty($_G[forum][extra][iconwidth]) && !empty($_G[forum][icon])} style="width: {$_G[forum][extra][iconwidth]}px;"{/if}>
								<!--{if $_G[forum][icon]}-->
									<a href="forum.php?mod=forumdisplay&fid=$_G[fid]"><img src="{$_G[setting][attachurl]}common/{$_G[forum][icon]}" alt="$_G[forum][name]" /></a>
								<!--{else}-->
									<a href="forum.php?mod=forumdisplay&fid=$_G[fid]"><img src="{$_G[style][styleimgdir]}/forum{if $_G[forum][todayposts]}_new{/if}.png" alt="$_G[forum][name]" /></a>
								<!--{/if}-->
								</div>
					<h1 class="xs2">
					...
					<div>
						<span class="y">
							<a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum&formhash={FORMHASH}" id="a_favorite" class="fa_fav" onclick="showWindow(this.id, this.href, 'get', 0);"><strong class="xi1" id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}>(<span id="number_favorite_num">$_G[forum][favtimes]</span>)</strong></a>
							<!--{if rssforumperm($_G['forum']) && $_G[setting][rssstatus] && !$_GET['archiveid'] && !$subforumonly}--><span class="pipe">|</span><a href="forum.php?mod=rss&fid=$_G[fid]&auth=$rssauth" class="fa_rss" target="_blank" title="RSS">&nbsp;</a><!--{/if}-->
							...
							<span class="pipe">|</span><a href="{if $_G['adminid'] == 1}admin.php?mod=forum&action=recyclebin&frames=yes{elseif $_G['forum']['ismoderator']}forum.php?mod=modcp&action=recyclebin&fid=$_G[fid]{/if}" class="fa_bin" target="_blank">&nbsp;</a>
							<!--{/if}-->
						...
						<!--{if $moderatedby}--><div class="moderatedby">{lang forum_modedby}: <span class="xi2">$moderatedby</span></div><!--{/if}-->
					</div>
				</div>
