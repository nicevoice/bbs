...
<!--{if $post[first] && $_GET['viewpid'] == $post['pid']}-->
...
<table id="pid$post[pid]" class="plhin plcom" summary="pid$post[pid]" cellspacing="0" cellpadding="0">
<tr>
	<!--{if !$close_leftinfo}-->
	...
	<!--{/if}-->
	<td class="plc plcon"<!--{if $close_leftinfo}--> style="width:100%"<!--{/if}-->>
	...
	<tr><td class="plc plm">
		...
		<!--{/if}-->
		<!--{if $post['first'] == 99 && $post['signature'] && ($_G['setting']['bannedmessages'] & 4 && ($post['memberstatus'] == '-1' || ($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5) || ($post['status'] & 1)))}-->
			<div class="sign">{lang member_signature_banned}</div>
		<!--{elseif $post['first'] == 99 && $post['signature'] && !$post['anonymous'] && $showsignatures}-->
			<div class="sign" style="max-height:{$_G['setting']['maxsigrows']}px;maxHeightIE:{$_G['setting']['maxsigrows']}px;">$post[signature]</div>
		<!--{elseif $post['first'] == 99 && !$post['anonymous'] && $showsignatures && $_G['setting']['globalsightml']}-->
...
<tr>
	...
	<td class="plc" style="overflow:visible;{if $close_leftinfo}--> width:100%{/if}">
		<div class="po hin">
			...
			<div class="pob cl">
				<em>
					<!--{if $post['first'] == 99 && $_G['group']['raterange'] && $post['authorid']}-->
					...
					<!--{/if}-->
					<!--{if $post['invisible'] == 0}-->
						<!--{if $post['first'] == 99 && $allowpostreply && $post['allowcomment'] && (!$thread['closed'] || $_G['forum']['ismoderator'])}--><a class="cmmnt" href="forum.php?mod=misc&action=comment&tid=$post[tid]&pid=$post[pid]&extra=$_GET[extra]&page=$page{if $_G['forum_thread']['special'] == 127}&special=$specialextra{/if}" onclick="showWindow('comment', this.href, 'get', 0)">{lang comments}</a><!--{/if}-->
