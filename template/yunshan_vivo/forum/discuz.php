...
<!--{eval $_G['setting']['forumallowside'] = TRUE;}-->
...
<!--{/if}-->
	...
	<div id="chart">
		<div class="wp cl">
			<div class="chart">
				<p class="chart_today"><i></i>{lang index_today}: <em>$todayposts</em></p>
				<p class="chart_yesterday"><i></i>{lang index_yesterday}: <em>$postdata[0]</em></p><p class="chart_posts"><i></i>{lang index_posts}: <em>$posts</em></p>
				<p class="chart_members"><i></i>{lang index_members}: <em>$_G['cache']['userstats']['totalmembers']</em></p>
				<!--{if $_G['cache']['userstats']['newsetuser']}-->
				<p class="chart_new"><i></i>{lang welcome_new_members}: <em><a href="home.php?mod=space&username={echo rawurlencode($_G['cache']['userstats']['newsetuser'])}" target="_blank" class="xi2">$_G['cache']['userstats']['newsetuser']</a></em></p>
				<!--{/if}-->
			</div>
			<!--div class="y">
					...
					<table cellspacing="0" cellpadding="0" class="fl_tb">
						...
						<!--{if $cat['forumcolumns']}-->
							...
							<td class="fl_g" width="$cat[forumcolwidth]">
								<div class="fl_icn_g"{if !empty($forum[extra][iconwidth]) && !empty($forum[icon])} style="width: {$forum[extra][iconwidth]}px;"{/if}<!--{if $forum[icon]}--> data-fid="{$forum[fid]}"<!--{/if}-->>
								<!--{if $forum[icon]}-->
									$forum[icon]
								<!--{else}-->
